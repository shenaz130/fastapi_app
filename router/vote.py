...
def vote( vote : schema.Vote, db:  Session = Depends(get_db), current_user :int = Depends(Oauth2.get_current_user)):
    ...
    vote_query = db.query(models.Vote).filter(
        models.Vote.tweet_id == vote.tweet_id, models.Vote.user_id == current_user.id)
    found_vote =vote_query.first()
    if (vote.dir == 1):
        if found_vote:
            raise HTTPException(status_code= status.HTTP_409_CONFLICT,
                                 detail= f"user {current_user.id} already voted the tweet number {vote.tweet_id}")
        new_vote = models.Vote(tweet_id=vote.tweet_id, user_id=current_user.id)
        db.add (new_vote)
        db.commit()
